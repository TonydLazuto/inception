...
wp-cli user create $DB_USER $DB_USER@4242.fr --role=administrator --user_pass=$DB_PASSWORD --allow-root
wp-cli user create $DB_USER2 $DB_USER2@4242.fr --role=editor --user_pass=$DB_PASSWORD2 --allow-root
wp-cli theme install https://downloads.wordpress.org/theme/colibri-wp.1.0.87.zip --activate --allow-root
